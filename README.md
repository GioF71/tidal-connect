...
As far as I know, content above 16/44 available as HI_RES quality (so 24/44 or 24/48) is currently inexistent on Tidal, so I guess we are now stuck to standard redbook resolution (16/44).  
...
The hires content from Tidal is not unavailable to some legacy streamers, including this (now quite outdated) implementation of Tidal Connect. Some of the [alternatives mentioned below](#alternatives) currently support Tidal HiRes streaming and will be able to play content up to 24/192.  
...
Keep in mind that the audio device index can also change because one time the (usb) device is already powered on during boot, and some other time it isn't.  
